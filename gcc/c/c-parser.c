...
c_parser_next_tokens_start_typename(c_parser *parser, enum c_lookahead_kind la)
{
	...
	if (la != cla_prefer_id && token->type == CPP_NAME && token->id_kind == C_ID_ID && (la == cla_prefer_type || c_parser_peek_2nd_token(parser)->type == CPP_NAME || c_parser_peek_2nd_token(parser)->type == CPP_MULT)
						...
						enum c_lookahead_kind la)
{
	...
	while (c_parser_next_token_is(parser, CPP_NAME) || c_parser_next_token_is(parser, CPP_KEYWORD))
	{
		struct c_typespec t;
		tree attrs;
		tree align;
	 ...
	 has simply forgotten a semicolon, so we exit.  */
		...
		if (c_parser_next_token_is(parser, CPP_NAME))
		{
	     ...
	     a C_ID_CLASSNAME.  */
			...
			else if (kind == C_ID_TYPENAME && (c_parser_next_token_is_not(parser, CPP_LESS)))
			{
				t.kind = ctsk_typedef;
							 ...
							 bool require_string, bool allow_empty_args)
{
	...
	if (c_parser_next_token_is(parser, CPP_NAME) && (c_parser_peek_token(parser)->id_kind == C_ID_ID) && ((c_parser_peek_2nd_token(parser)->type == CPP_COMMA) || (c_parser_peek_2nd_token(parser)->type == CPP_CLOSE_PAREN)) && (takes_identifier))
	{
